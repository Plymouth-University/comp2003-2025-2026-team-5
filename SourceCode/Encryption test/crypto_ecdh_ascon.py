...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "lib"))# crypto_ecdh_ascon.py
...
from lib.utinyec import registry
from lib.utinyec.ec import Point
...
def derive(shared_point):
    x = shared_point.x.to_bytes(32, "big")
...
class Session:
    def __init__(self, my_priv, peer_pub):
        self.my_priv = my_priv
        self.peer_pub = peer_pub

        shared = compute_shared(my_priv, peer_pub)
        key, _ = derive(shared)
        self.key = key  # 16‑byte ASCON key

    def encrypt(self, msg):
        nonce = randbytes(16)
        ct, tag = ascon_encrypt(self.key, nonce, msg)
        return ct, tag, nonce

    def decrypt(self, ct, tag, nonce):
        return ascon_decrypt(self.key, nonce, ct, tag)



def server_mode():
    # fresh server keypair
    srv_priv, srv_pub = gen_ecdh()

    # send server public key as first line
    ...
    # then decrypt each incoming JSON line
    for raw in sys.stdin:
        ...
        dev_pub_x = int(data["dev_pub_x"])
        dev_pub_y = int(data["dev_pub_y"])
        dev_pub = Point(curve, dev_pub_x, dev_pub_y)

        # create session on first message, reuse after
        if session is None:
            session = Session(srv_priv, dev_pub)

        nonce = bytes.fromhex(data["nonce"])
        ct = bytes.fromhex(data["ciphertext"])
        tag = bytes.fromhex(data["tag"])

        pt = session.decrypt(ct, tag, nonce)
        sys.stdout.write(pt.decode("ascii", "ignore") + "\n")
        sys.stdout.flush()
